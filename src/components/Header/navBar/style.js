...
export const getStyles = (isActiveBurger) => ({
    ...
    burgerMenu: clsx(
        "absolute flex items-center justify-center right-0 top-0 z-50 duration-700  shadow-2xl  after:rotate-90 after:bottom-0 before:rotate-90 before:top-0 ",
        { "w-full h-screen bg-blue z-50 ": isActiveBurger },
        { "w-[100px] h-[100px] rounded-bl-full bg-aqua": !isActiveBurger }
    ),
    navigation: clsx(
        "transition delay-300 duration-700 ease-in-out  text-3xl p-2 w-full flex flex-col items-center justify-center ",
        {
            "h-fit opacity-100 ": isActiveBurger,
        },
        {
            "h-0 overflow-hidden opacity-0 delay-0 duration-300":
                !isActiveBurger,
        }
    ),
    burger: clsx(
